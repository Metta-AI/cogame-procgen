...
# use-after-free fixed in procgen_replay.nim (emscripten_exit_with_live_runtime)
...
# procgen_error_len(). Aborting keeps linear memory intact, and the page reads
# procgen_stage_ptr/len afterwards to report what the runtime was doing.
switch(
  "passL",
  (&"""
  -o {distDir / "procgen_replay.js"}
  ...
  -s EXPORTED_FUNCTIONS=_main,_malloc,_free,_procgen_load_replay,_procgen_frame,_procgen_input,_procgen_packet_ptr,_procgen_packet_len,_procgen_mismatch_tick,_procgen_error_ptr,_procgen_error_len,_procgen_stage_ptr,_procgen_stage_len
  """).replace("\n", " ")
)
